...
def summarize_fld(dateStamp,pathDir,destDir,type='count'):
    """
    ...
    if type == 'count':  header = '\t'.join(['FULL PATH','SUBDIR','CH','PEAK COUNT'])
    if type == 'intensity': header = '\t'.join(['FULL PATH', 'SUBDIR','CH','MEAN PEAK INTENSITY'])
    ofile_long.write(header+'\n')

    ofile_short = open(out_short,'w')
    if type == 'count':  header = '\t'.join(['FULL PATH','SUBDIR','CH','AVERAGE PEAK COUNT', 'STDEV PEAK COUNT'])
    if type == 'intensity': header = '\t'.join(['FULL PATH','SUBDIR','CH', 'AVERAGE of MEAN PEAK INTENSITY','STDEV MEAN INTENSITY'])
    ...
    for f in fList:
        ...
        if type == 'count':  colVal = len(peakInfo) #Just this for now. Simples
        if type == 'intensity': colVal = getMeanIntensity(peakInfo)
...
#    allDir = [subDir for subDir in next(os.walk(pathDir))[1] if 'trace' in subDir]
    ...
    if ARG == 'FLDS':
        summarize_fld(dateStamp,pathDir,destDir,type=val)
